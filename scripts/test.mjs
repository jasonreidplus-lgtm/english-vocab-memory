...
t('dueReviewIds：到期才出', () => {
  ...
  const prog = baseProgress({ wrong: { 5: { miss: 1, card: due }, 6: { miss: 1, card: future } } });
...
t('覆盖/分布/到期 基本正确', () => {
  const prog = baseProgress({ levels: { 1: { completed: true, stars: 3, bestScore: 9, attempts: 1 } }, stats: { answered: 10, correct: 8 } });
  ...
  assert.equal(s.retention.current, null);
...
t('错词进入学习中档 + 有保持率', () => {
  ...
  const prog = baseProgress({ levels: { 1: { completed: true, stars: 3, bestScore: 9, attempts: 1 } }, wrong: { 5: { miss: 1, card: c } } });
...
function baseProgress(over) {
  return {
    ...
    levels: {}, wrong: {}, daily: null, history: {}, newHistory: {}, revlog: [],
    stats: { answered: 0, correct: 0 }, sound: true, accent: 'us', examDate: '2026-12-21',
    ...over,
  };
}
