...
> 从旧版临时 CI 签名切换到 v1.1.0 时，旧 APK 可能无法直接覆盖。请先在 APP 设置中点「跨设备同步 → 复制」，把完整备份码存到微信收藏/记事本，再卸载旧版、安装新版，最后粘贴导回。工作流从 v1.1.0 起会缓存并每周保活同一把调试签名；v1.2.0 继续沿用这一签名，可从 v1.1.0 直接覆盖安装。
